...
exports.monitorEmailAccountRefactor = monitorEmailAccountRefactor;
exports.cleanupOldProcessedEmails = cleanupOldProcessedEmails;
...
function connectRetryable(err, isLast) {
    if (isLast)
        return false;
    if (err?.authenticationFailed) {
        return false;
    }
    return (0, imap_transient_1.isTransientImapError)(err);
}
async function safeNotifyConnectionError(email, error) {
    try {
        await discord_notification_1.discordNotification.notifyEmailConnectionError(email, error);
    }
    catch (notifyErr) {
...
async function monitorEmailAccountRefactor(email, password, onNewMail) {
    let client = null;
    try {
        for (let attempt = 0; attempt < config_1.config.monitoring.imapConnectMaxAttempts; attempt++) {
            if (client) {
                try {
                    await client.logout();
                }
            ...
            try {
                ...
                const isLast = attempt === config_1.config.monitoring.imapConnectMaxAttempts - 1;
                if (connectRetryable(connectErr, isLast)) {
                    const delay = config_1.config.monitoring.imapConnectBaseDelayMs * Math.pow(2, attempt);
                    console.warn(`⚠️ IMAP ${email} tentativa ${attempt + 1}/${config_1.config.monitoring.imapConnectMaxAttempts} falhou; nova tentativa em ${delay / 1000}s:`, connectErr);
        ...
        const messages = client.fetch({ since: windowStart }, {
            envelope: true,
            source: true,
            uid: true,
        });
        ...
        for await (const msg of messages) {
            try {
                ...
                console.error(`❌ Erro ao fazer parse da mensagem UID ${msg.uid}:`, error);
            }
        }
        console.log(`📨 ${email}: Encontrados ${allEmails.length} e-mail(s) na janela de ${fetchWindowMs / 3600000} h`);
        if (allEmails.length === 0) {
            return;
        }
        // Busca quais já foram processados
        const processedEmails = await prisma_1.prisma.processedEmail.findMany({
            where: {
                accountEmail: email,
                OR: [
                    {
        ...
        const newEmails = allEmails.filter((mail) => {
            const hasBeenProcessed = processedMessageIds.has(mail.messageId) || processedUIDs.has(mail.uid);
        ...
        if (allEmails.length > 0 && newEmails.length === 0) {
            for (const mail of allEmails) {
                const processed = processedMessageIds.has(mail.messageId) ||
                ...
                const withinCooldown = lastFailAt
                    ...
                    : false;
                if (processed) {
                    console.log(`↪️ ${email}: ${mail.messageId} (UID ${mail.uid}) ignorado: já processado`);
                }
                else if (withinCooldown && lastFailAt) {
                    const nextTryAt = new Date(lastFailAt + FAILURE_COOLDOWN_MS).toLocaleTimeString();
                    console.log(`↪️ ${email}: ${mail.messageId} (UID ${mail.uid}) ignorado: em cooldown até ${nextTryAt}`);
                }
                else {
                    console.log(`➡️ ${email}: ${mail.messageId} (UID ${mail.uid}) elegível, porém não selecionado (verificar lógica)`);
                }
            }
        }
        for (const emailData of newEmails) {
            try {
                ...
                await prisma_1.prisma.processedEmail.create({
                    data: {
                ...
                console.error(`❌ Erro ao processar email ${emailData.messageId}:`, error);
        ...
        const err = error;
        if (err?.authenticationFailed) {
            console.error(`❌ IMAP autenticação falhou: ${email}\n` +
...
async function cleanupOldProcessedEmails(daysToKeep = 30) {
    const cutoffDate = new Date(Date.now() - daysToKeep * 24 * 60 * 60 * 1000);
    const deleted = await prisma_1.prisma.processedEmail.deleteMany({
        where: {
            processedAt: {
                lt: cutoffDate,
            },
    ...
    console.log(`🧹 Removidos ${deleted.count} registros de emails antigos (mais de ${daysToKeep} dias)`);
    return deleted.count;
}
// Função para obter estatísticas
async function getProcessingStats(email) {
    const where = email ? { accountEmail: email } : {};
    const stats = await prisma_1.prisma.processedEmail.aggregate({
        where,
        _count: { id: true },
